...
int main() {
    ...
    for (int i = 1; i < n; i++) {
        ...
        /**
         * So lemme explain what is happening here. I used a part from face recognition algorithm from https://youtu.be/uEJ71VlUmMQ?t=395
         * The approach is to have sum of all inputs from 0 to n stored in an array like input_sums. In this case, if I want to find out
         ...
         * I want to find out what the sum of elements 4(last element) to 2. (3 + 4 + 5 = 12). All I have to is to subtract 15 - 3 = 12
         *
         * Before submitting the this code, I used to have a another for loop to do like:
         * for(int i = 0; i < n; i++)
         *  for(int j = i + 1; j < n; j++)
         *   sum += input_sums[j] - input_sums[i];
         * This nested loops used to calculate all f(l,r)s possible
         * That algorithm was O(n^2) and thus used to fail on big n (test 10 to 13)
         *
         * Next, I wrote a code here (in this loop) to reduce the LOC. The approach was this:
         * sum += input_sums[i];
         * for(int j = 0; j < i; j++)
         *  sum += input_sums[i] - input_sums[j];
         * This algorithm was O(n^2) and this failed in big n
         *
         ...
         * So why not use another sum array for this one?"
         *
